...
namespace XHTMLClassLibrary.Attributes
{
    public class ValuesSelectionTypeAttribute<T> : SimpleSingleTypeAttribute<T> where T : IAttributeDataType, new()
    {
            ...
            set
            {
                if (value != null)
                {
                    ...
                    if (!(_dataType.IsInstanceOfType(value)) && !(value is string))
                        throw new ArgumentException(string.Format("The value set can be only of string or {0} type",
                            _dataType.Name));
                    ...
                    if (_possibleValues.Count > 0)
                    {
                        if (!_possibleValues.Contains(passedValue))
                        {
                            throw new ArgumentException(string.Format(
                                "The value {0} is not one of the possible values", passedValue));
                        }
                    }
        ...
        {
            BaseAttribute clonedAttribute = (BaseAttribute)Activator.CreateInstance(GetType(), GetAttributeName(), _dataType);
